...
describe("A1: a failure says what to do next", () => {
  ...
  it("keeps one sentence for every cause, which is the enumeration property", () => {
    // OWASP A07 asks for the same message for all outcomes, not for no message.
    // A second published failure code would have to be justified against this.
    expect(Object.values(AUTH_OUTCOMES)).toHaveLength(1)
  })
