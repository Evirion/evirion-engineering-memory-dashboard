...
MEM-UX/05, MEM-UX/07, MEM-UX/03, and MEM-UX/06 are merged on `main` (`3979c36`).
MEM-UX/04 is implemented and locally verified on
`MEM-UX/04-knowledge-detail-decision-first`. MEM-UX/01 and MEM-UX/02 stay
planned. Nothing from this plan is deployed or observed on staging except where
earlier milestones already landed.
