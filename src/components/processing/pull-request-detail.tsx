  ...
  return (
    <div className="flex flex-col gap-8">
      <section aria-label="Pull request summary" className="flex flex-col gap-2">
        ...
        {detail.title ? (
          <p className="text-sm text-ink-secondary">{detail.title}</p>
        ) : null}
        <p className="text-sm text-ink-secondary">State: {detail.currentState}</p>
        {detail.authorLogin ? (
      ...
      <section aria-label="Admitted knowledge" className="flex flex-col gap-2">
        <h2 className="text-sm font-semibold text-foreground">Admitted knowledge</h2>
        {detail.admittedKnowledgeObjects.length === 0 ? (
          <p className="text-sm text-ink-secondary">No admitted knowledge objects.</p>
        ) : (
          <ul className="flex flex-col gap-2 text-sm">
            {detail.admittedKnowledgeObjects.map((object) => (
              <li key={object.knowledgeObjectId}>
                ...
                <span className="text-ink-secondary"> — {object.knowledgeType}</span>
              </li>
            ))}
          </ul>
        )}
